...
!!! USING SCRIPTS INDIVIDUALLY (NOT USING THE MENU FIRST) WILL RESULT IN AN ERROR THAT CAN BE SAFELY IGNORED AS THE SCRIPTS WILL TRY TO RETURN TO THE MENU UPON COMPLETION
...
All scripts will include ```#!/bin/bash``` as they are written to be interpreted by bash. As long as bash is present on your system they should run fine regardless of interactive shell.
...
!!! THIS SCRIPT MAY EDIT  `/usr/share/icons/default` WHICH CAN CAUSE AN INCORRECT CURSOR THEMES FOR OTHER USERS ON THE SYSTEM
!!! IF THERE ARE OTHER USERS WHO LOG INTO YOUR SYSTEM, READ THE SCRIPT CAREFULLY BEFORE EXECUTING SO YOU DON'T BREAK THEIR THEMES
...
!!! THIS SCRIPT IS INTENDED FOR A PROPER PULSEAUDIO/JACK BRIDGED SETUP AND MAY NOT WORK UNDER PIPEWIRE. IT WILL NOT WORK ON A SOLELY PULSEAUDIO OR JACK SETUP.
...
!!! THIS SCRIPT IS DEPRECATED AS NOW INCLUDES A V4L2LOOPBACK VIRTUAL CAMERA OUT OF THE BOX
